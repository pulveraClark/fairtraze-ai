...
- **.docx import (planned, not built)** — a `.docx` upload feature for offline-drafting students is designed but not implemented. Only schema groundwork exists today: `EditEvent`/`EditSession` have a `source: LIVE | IMPORT` column (`server/prisma/schema.prisma`), defaulted to `LIVE` and currently produced by nothing but `LIVE` — no route, parser, UI, or scoring-formula change exists yet. Do not build on the assumption that import scoring is designed-and-pending-implementation only; the scoring formula itself (synthetic session credit from import volume, a disclosure note) is decided but not coded.
